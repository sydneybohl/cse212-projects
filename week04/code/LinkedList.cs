...
public class LinkedList : IEnumerable<int>
{
    ...
    public void Remove(int value)
    {
        ...
        // Searching for the node that matches 'value' by starting at the head
        Node? curr = _head.Next;
        while (curr is not null)
        {
            if (curr == _tail && curr.Data == value)
            {
                // If the location of 'value' is at the end of the list,
                // then we can call RemoveTail to remove it
                RemoveTail();
                return; // Stop after removing the node
            }
            
            if (curr.Data == value)
            {
                // Reconnect the previous and next nodes to remove 'curr' from the list
    ...
    {
        // TODO Problem 5
        var curr = _tail; // Starting at the end since this is a backward iteration.
        while (curr is not null)
        {
            yield return curr.Data; // Provide (yield) each item to the user
